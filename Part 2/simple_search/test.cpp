...
double get_prob(double loss,double k, double max_k){
    double probability;
    double temerature=1-((k+1)/max_k);
    cout<<"temperature: "<<temerature<<endl;
    probability=exp(-(loss/temerature));
    return probability;
}
